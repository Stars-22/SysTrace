...
[![Release](https://img.shields.io/badge/Release-v1.0.0-green.svg)](../../releases)
...
- **Auto-refresh** — Configurable intervals (1s, 2s, 5s, 1m, 5m, 10m) for continuous monitoring
...
g++ -std=c++20 -O2 -static -o SysTrace.exe main.cpp src/*.cpp -Isrc -Ithird_party/httplib -lpsapi -lkernel32 -lws2_32 -lpdh
```
...
| System API | Windows kernel32, psapi, pdh | CPU/memory/disk/process collection |
...
- **自动刷新** — 可选 1s/2s/5s/1m/5m/10m 自动刷新，持续监控
...
| 系统 API | Windows kernel32, psapi, pdh | CPU/内存/磁盘/进程采集 |
